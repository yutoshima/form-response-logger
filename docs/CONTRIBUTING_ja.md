...
- **問題表示モード選択**: HTML表示とプレーンテキスト表示を設定で切り替え可能
  - HTML表示ON: 短文用、HTML書式設定（太字、色など）が使える
  - HTML表示OFF: 長文用、縦スクロール対応のプレーンテキスト表示
  - Config.javaのuseHtmlRenderingフィールドで管理

- **問題のHTML対応**: リッチテキスト書式設定のためのHTMLタグをサポート（HTML表示ONの場合のみ）
  - 太字（`<b>`）、斜体（`<i>`）、下線（`<u>`）
  - 改行（`<br>`）、段落（`<p>`）
  - カスタム書式設定のためのCSSスタイル
